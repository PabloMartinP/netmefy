package ar.com.netmefy.netmefy.services.api.entity;

/**
 * Created by fiok on 12/10/2017.
 */

public class testModel {
    //{ cliente_sk: 1, ot_id: 1, flag_instalacion: 1, comentario:'Test de OT', vel_mb_medidos: 25, potencia_recep: 100, flag_cableado:1, flag_modem_ok:1  };
    public int cliente_sk;
    public int ot_id;
    public int flag_instalacion;
    public String comentario;
    public int vel_mb_medidos;
    public int potencia_recep;
    public  int flag_cableado_nuevo;
    public int flag_modem_ok;

}
